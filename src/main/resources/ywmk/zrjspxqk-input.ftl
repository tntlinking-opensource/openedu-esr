...
<#if id?exists>
	<#assign extPosition='修改专任教师培训情况'/>
<#else>
	<#assign extPosition='创建专任教师培训情况'/>
</#if>
			...
			<div class="span12">
				...
				<div class="span12">
					<div class="portlet box green">
						...
						<!--内容表单-->
						<form id="save_inputForm" class="form-horizontal" action="${base}/ywmk/zrjspxqk!save.action" method="post">
							<@s.token />
							<#if id?exists>
							...
							<div class="control-group">
								<label class="control-label"><b>培训名称:</b><span class="required">*</span></label>
								<div class="controls">
									<input type="text" class="span10 m-wrap" id="pxmc" name="pxmc" value="${pxmc!}" maxlength="100" />
								</div>
							</div>
							<div class="control-group">
								<label class="control-label"><b>学时:</b><span class="required">*</span></label>
								<div class="controls">
									<input type="text" class="span10 m-wrap" id="xueshi" name="xueshi" value="${xueshi!}" maxlength="20" />
								</div>
							</div>
							<div class="control-group">
								<label class="control-label"><b>培训层次:</b></label>
								<div class="controls">
									<select name="pxcc.id">
										<option value="">==请选择==</option>
										<#if reqlist?exists>
											<#list reqlist as obj>
												<option value="${obj.id}" <#if pxcc?exists && pxcc.id?exists && pxcc.id == obj.id>selected</#if> >${obj.name!}</option>
											</#list>
										</#if>
									</select>
								</div>
							...
							<div class="control-group">
								<label class="control-label"><b>培训年月:</b><span class="required">*</span></label>
								<div class="controls">
									<input type="text" class="span10 m-wrap" id="pxny" name="pxny" value="${pxny!}" readonly onclick="WdatePicker({el:'pxny',dateFmt:'yyyy-MM',readOnly:true})" maxlength="20" />
								</div>
							</div>
