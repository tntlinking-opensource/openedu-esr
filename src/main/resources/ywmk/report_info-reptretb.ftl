...
<@crudmetropage3>
	...
	<div>
		...
		<p/>
			...
			<#if reptTable?exists && reptTable?length gt 0 >
				${reptTable}
			 <#else>
			 	${entity?if_exists.reptTable!}
			</#if>
			
			
	</div>
	<div style="position: fixed;bottom: 700px;width: 97%;text-align: right;">
		<button class="btn big blue" type="button" onclick="save()">保存</button>
		&nbsp;
		&nbsp;
		<button class="btn big blue" type="button" onclick="yjb0()">一键补0</button>
	</div>	
	...
	<script>
		...
		function save(){
	        ...
	        if(rwid != '' && rwid != null ){
				...
				form.attr("action","${base}/sxglpt/report_info!saveUserReport.action");
				...
				$("#reptTable").appendTo(form);
		   		...
		   		$("<input type='hidden' value='"+rwid+"' name='rwid'>").appendTo(form);  
		...
		$(document).ready(function() {
			...
			<#else>
				...
				$("td[typ=numb]").each(function(){
					...
					var ywhyystr = "#td_1_"+(ystr);
					ywhyystr=$(ywhyystr).html();
					if(ywhyystr != null && ywhyystr!= '' &&  typeof(ywhyystr) != 'undefined'){
						ywhyystr = del_html_tags(ywhyystr.replace(/&nbsp;/g,''));
					}else{
						ywhyystr = "";
					}
