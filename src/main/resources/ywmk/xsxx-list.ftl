...
<@crudmetropage3>
	<form id="query_form" action="${base}/ywmk/xsxx.action" method="post">
		...
		<div class="row-fluid cpquery">
				<div class="span1">
			  	...
			  	<div class="span1">
					...
					<input type="text" class="m-wrap span12" id="filter_LIKES_xh" name="filter_LIKES_xh"  value="${Parameters['filter_LIKES_xh']?if_exists}" maxlength="50"/>
			  	</div>
			  	<div class="span1">
			  		<label class="control-label">
						姓名:
					</label>
					<input type="text" class="m-wrap span12" id="filter_LIKES_xm" name="filter_LIKES_xm"  value="${Parameters['filter_LIKES_xm']?if_exists}" maxlength="100"/>
			  	</div>
			  	<div class="span1">
			  		<label class="control-label">
						性别:
				...
				<div class="span2">
					...
					<br/>
					<input readonly id="filter_GES_rxnf" name="filter_GES_rxnf" class="span5 m-wrap" value="${Parameters['filter_GES_rxnf']?if_exists}" type="text" onclick="WdatePicker({el:'filter_GES_rxnf',dateFmt:'yyyy',readOnly:true})">
					至
					<input readonly id="filter_LES_rxnf" name="filter_LES_rxnf" class="span5 m-wrap" value="${Parameters['filter_LES_rxnf']?if_exists}" type="text" onclick="WdatePicker({el:'filter_LES_rxnf',dateFmt:'yyyy',readOnly:true})">
				</div>
				<div class="span2">
					<label class="control-label">
						班级:
					...
					<select name="filter_EQS_ssbj.id" id="filter_EQS_ssbj.id" class="m-wrap span10">
						<option value="">==全部==</option>
						<#if BanjiList?exists>
							<#list BanjiList as obj>
								<option value="${obj.id!}"  <#if Parameters['filter_EQS_ssbj.id']?exists && Parameters['filter_EQS_ssbj.id'] == obj.id>selected</#if> >${obj.bjmc!}</option>
							</#list>
						</#if>
					</select>
				</div>
							...
							<div class="portlet-body">
								...
								<#list page.result as obj>
									...
									<tr titl="${obj.id}">
										...
										<td>
											${obj.ssbj?if_exists.bjmc!}
										</td>
									  	<td>
									  		${obj.mz?if_exists.name!}
									  	</td>
									  	<td>
									  		${obj.zzmm?if_exists.name!}
									  	</td>
									  	<td>
											${obj.jig?if_exists.name!}
									  	</td>
										 <td>
											<center>
									 			<a class="oprt-a" href="javascript:;" onclick="window.open('${base}/ywmk/xsxx!input.action?id=${obj.id}')"> 修改</a>
											</center>
										 </td>
									</tr>
								</#list>
								...
								<script type="text/javascript">
									...
									function add()
									{
											window.open('${base}/ywmk/xsxx!input.action');
									}
									...
									function leadingExcel() {
										...
										parent.layer.open({
											type: 2,
											title: ' ',
											content: '${base}/ywmk/xsxx!tmimp.action',
									...
									$('#query_form').pfwpage({
										...
										page_button:[
											...
											{b_name:'删除',opttype:'delete',bclass:'btn redbtn',bicon:'icon-trash',b_url:'${base}/ywmk/xsxx!delete.action',confirmStr:'是否确认删除？',paramName:'checks',selectNum:'^[0-9]*[1-9][0-9]*$'}
										],
