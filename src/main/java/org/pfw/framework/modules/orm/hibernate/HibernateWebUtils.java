package org.pfw.framework.modules.orm.hibernate;
...
import org.pfw.framework.modules.utils.ReflectionUtils;
...
public class HibernateWebUtils {
	...
	public static Map getTableval(final HttpServletRequest request) {
		return getTabvByReq(request, "ipt_");
	}
 
	public static Map getTabvByReq(final HttpServletRequest request, final String filterPrefix) {

		//从request中获取含属性前缀名的参数,构造去除前缀名后的参数Map.
		Map filterParamMap = getParametersStartingWith(request, filterPrefix);
	...
	public static List<PropertyFilter> buildPropertyFilters(final HttpServletRequest request, final String filterPrefix) {
		...
		if(!filterParamMap.isEmpty()){
			Set keys = filterParamMap.keySet();
			Iterator it = keys.iterator();
			while(it.hasNext()){
				String filterName = (String)it.next();
				Object ob = filterParamMap.get(filterName);
				if(ob instanceof String)
				{
					String value = (String)filterParamMap.get(filterName);
				...
				{
					List value = Arrays.asList((String[])filterParamMap.get(filterName));
