...
@Results
	({ 
		...
		@Result(name="success",location="/ywmk/zyxx-list.ftl",type = "freemarker"),
		@Result(name="tmimp",location="/ywmk/zyxx-daor.ftl",type = "freemarker"),
		@Result(name ="input",location="/ywmk/zyxx-input.ftl", type = "freemarker"),
...
public class
ZyxxAction extends CrudActionSupport<Zyxx> {
	@Autowired
	protected ZyxxService entityService;
	@Autowired
	...
	protected void prepareModel() throws Exception {
		if (!StringUtils.isEmpty(id)) {
			entity = (Zyxx) entityService.getById(id);
		} else {
			entity = new Zyxx();
		}
	...
	{
		try {
			String  mbname	="专业信息Excel模板.xls";
			String scdz = "/zyxxdr.xls";
	...
	public String daorxx() throws Exception{
		...
		try {
			...
			for(int row = 1; row < sheet.getLastRowNum()+1; row++){
				...
				if(ok){
					List<Zyxx> zyxxList = entityService.findbyhql(" from Zyxx r where r.zydm = ?", xm.getZydm());
					if (zyxxList != null && zyxxList.size() > 0){
						Zyxx bjxx = zyxxList.get(0);
						xm.setId(bjxx.getId());
