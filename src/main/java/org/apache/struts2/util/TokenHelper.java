    ...
    */
    ...
    public static boolean validToken(){
    	...
    	if (tokenName != null) {
    		...
    		Map req = (Map<String,Object>)ActionContext.getContext().get("request");
        	if(req != null)
        	{
        		ActionMapping actionMap = (ActionMapping)req.get("struts.actionMapping");
        		if(actionMap != null)
        		{
        			String actionmethod = (String)actionMap.getMethod();
        			if(StringUtils.isNotEmpty(actionmethod))
        			{
        				if(actionmethod.indexOf("save") > -1 || actionmethod.indexOf("update") > -1){
        					if(!(actionmethod.indexOf("savePassword") > -1) && !(actionmethod.indexOf("updatePassword") > -1))
        						return false;
        				}
        			}
        		}
